...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* Main Navbar */}
      <div
        className={`border-b-[1px] hidden lg:block ${
          ...
          <div className="flex flex-row justify-between items-center">
            <div>
              <Link
                ...
                className="font-bold text-[30px] font-tomorrow"
              >
            ...
            >
              <ul className="flex flex-row xl:gap-x-[25px] items-center lg:gap-x-[15px]">
                {navItems.map((item, index) => (
                  <motion.li
                    key={index}
                    className={`text-[18px] font-medium font-tomorrow transition duration-300 ease-in-out ${
                      activeLink === item.path ? "text-[#656565]" : "text-black"
                    } hover:text-[#656565] hover:scale-105 ${
                      item.name === "Contact Us"
                        ? "bg-[#1D4665] px-4 py-2 rounded"
                        : "text-black"
                    }`}
                  ...
                  >
                    <Link
                      to={item.path}
                      className={`${
                        item.name === "Contact Us" ? "text-white" : ""
                      }`}
                    >
                      {item.name}
                    </Link>
      ...
      <div
        className="lg:hidden block px-[20px] py-[25px] border-b-[1px] border-black sticky top-0 bg-white z-[9999]"
        style={{ transition: "top 0.3s ease, position 0.3s ease" }}
      >
        <div className="flex justify-between  items-center">
          ...
          <div>
            <button onClick={toggleDrawer}>
              <img src={menuicon} className="w-[32px]" />
            </button>
          </div>
        </div>
